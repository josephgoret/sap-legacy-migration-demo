...
class VendorDetail(BaseModel):
    ...
    vendor_number: str = Field(max_length=10, description="Vendor account number (LIFNR)")
    name1: str = Field(max_length=35, description="Name line 1 (NAME1)")
    name2: Optional[str] = Field(default=None, max_length=35, description="Name line 2 (NAME2)")
    street: Optional[str] = Field(default=None, max_length=35, description="Street address (STRAS)")
    city: Optional[str] = Field(default=None, max_length=35, description="City (ORT01)")
    region: Optional[str] = Field(default=None, max_length=3, description="Region/state (REGIO)")
    postal_code: Optional[str] = Field(default=None, max_length=10, description="Postal code (PSTLZ)")
    country: Optional[str] = Field(default=None, max_length=3, description="Country key (LAND1)")
    phone: Optional[str] = Field(default=None, max_length=16, description="Telephone (TELF1)")
    fax: Optional[str] = Field(default=None, max_length=31, description="Fax number (TELFX)")
    email: Optional[str] = Field(
        default=None, max_length=241, description="Email address (ADR6-SMTP_ADDR)"
    )
    account_group: Optional[str] = Field(
        default=None, max_length=4, description="Vendor account group (KTOKK)"
    )
    payment_terms: Optional[str] = Field(
        default=None, max_length=4, description="Payment terms key (ZTERM)"
    )
    recon_account: Optional[str] = Field(
        default=None, max_length=10, description="Reconciliation account (AKONT)"
    )
...
class PurchaseOrderItem(BaseModel):
    ...
    po_number: str = Field(max_length=10, description="Purchase order number (EBELN)")
    po_item: str = Field(max_length=5, description="PO item number (EBELP)")
    po_date: date = Field(description="PO document date (BEDAT)")
    material_number: Optional[str] = Field(
        default=None, max_length=18, description="Material number (MATNR)"
    )
    short_text: Optional[str] = Field(
        default=None, max_length=40, description="Item short text (TXZ01)"
    )
    quantity: Decimal = Field(description="Order quantity (MENGE)")
    unit_of_measure: str = Field(max_length=3, description="Unit of measure (MEINS)")
    ...
    purchase_requisition: Optional[str] = Field(
        default=None, max_length=10, description="Purchase requisition number (BANFN)"
    )
...
class VendorLookupRequest(BaseModel):
    ...
    vendor_number: str = Field(max_length=10, description="Vendor number (IV_LIFNR)")
    company_code: str = Field(
        default="1000", max_length=4, description="Company code (IV_BUKRS)"
    )
