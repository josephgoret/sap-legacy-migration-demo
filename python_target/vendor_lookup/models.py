...
class VendorDetail(BaseModel):
    ...
    name1: str = Field(max_length=100, description="Name line 1 (NAME1)")
    name2: Optional[str] = Field(default=None, max_length=100, description="Name line 2 (NAME2)")
    street: Optional[str] = Field(default=None, max_length=200, description="Street address (STRAS)")
    city: Optional[str] = Field(default=None, max_length=100, description="City (ORT01)")
    ...
    country: Optional[str] = Field(default=None, max_length=5, description="Country key (LAND1)")
    ...
    email: Optional[str] = Field(
        default=None, max_length=200, description="Email address (ADR6-SMTP_ADDR)"
    )
...
class PurchaseOrderItem(BaseModel):
    ...
    short_text: Optional[str] = Field(
        default=None, max_length=200, description="Item short text (TXZ01)"
    )
    quantity: Decimal = Field(description="Order quantity (MENGE)")
    unit_of_measure: str = Field(max_length=10, description="Unit of measure (MEINS)")
...
class VendorLookupResponse(BaseModel):
    ...
    return_message: str = Field(max_length=500)
