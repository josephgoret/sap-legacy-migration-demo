...
def lookup_vendor(
    ...
    po_data: list[dict],
    authorize: Optional[Callable[[str], bool]] = None,
) -> VendorLookupResponse:
    ...
    Args:
        ...
        authorize:   Optional callback that checks whether the caller is
                     authorized for the given company code.  Returns True
                     if access is granted.  Mirrors the ABAP
                     AUTHORITY-CHECK OBJECT 'F_LFA1_BUK' call.

    Returns:
        VendorLookupResponse with vendor details and PO history.

    Raises:
        AuthorizationError: If the authorize callback denies access.
        VendorNotFoundError: If vendor_data is None.
    """
    # Authorization check — maps to ABAP: AUTHORITY-CHECK OBJECT 'F_LFA1_BUK'
    if authorize is not None and not authorize(request.company_code):
