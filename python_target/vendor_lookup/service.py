...
def lookup_vendor(
    ...
    Args:
        ...
        auth_checker: Callback that receives the company code and returns
                      True if the caller is authorized.  Maps to ABAP
                      AUTHORITY-CHECK OBJECT 'F_LFA1_BUK'.  When None
                      the check is skipped (for tests / trusted internal
                      callers only).
