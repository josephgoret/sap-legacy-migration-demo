...
class OrderHeader(BaseModel):
    ...
    incoterms2: Optional[str] = Field(
        default=None, max_length=80, description="Incoterms part 2 (location)"
    )
...
class InboundOrderMessage(BaseModel):
    ...
    message_id: str = Field(max_length=40, description="Unique message ID (replaces IDoc DOCNUM)")
    message_type: str = Field(
        default="ORDERS", max_length=20, description="Message type (replaces MESTYP)"
    )
    sender_system: Optional[str] = Field(
        default=None, max_length=40, description="Sending system identifier"
    )
...
class OrderSyncResult(BaseModel):
    ...
    message_id: str = Field(max_length=40)
    status: OrderStatus
    order_number: Optional[str] = Field(
        default=None, max_length=20, description="Created sales order number (VBELN)"
    )
