...
class OrderPartner(BaseModel):
    ...
    number: str = Field(max_length=10, description="Partner number (PARTN)")


class OrderItem(BaseModel):
    """Order line item — maps to E1EDP01 + E1EDP19 segments / BAPISDITM."""

    item_number: str = Field(max_length=6, description="Item number (POSEX)")
    material_number: Optional[str] = Field(
        default=None, max_length=18, description="SAP material number from E1EDP19 qualifier 002"
    )
    customer_material: Optional[str] = Field(
        default=None, max_length=35, description="Customer material number from E1EDP19 qualifier 003"
    )
    plant: Optional[str] = Field(default=None, max_length=4, description="Delivering plant")
    quantity: Decimal = Field(description="Order quantity (MENGE)")
    unit_of_measure: str = Field(default="EA", max_length=3, description="Unit of measure (MENEE)")
    net_price: Decimal = Field(
        default=Decimal("0"), description="Net price (VPREI)"
    )
    item_category: Optional[str] = Field(
        default=None, max_length=4, description="Item category (PSTYV)"
    )
...
class OrderHeader(BaseModel):
    ...
    document_type: str = Field(
        default="ZOR", max_length=4, description="Sales document type (BSART/AUART)"
    )
    ...
    ship_to_party: Optional[str] = Field(
        default=None, max_length=10, description="Ship-to customer number"
    )
    customer_po_number: Optional[str] = Field(
        default=None, max_length=20, description="Customer PO reference (BSTKD)"
    )
    ...
    incoterms1: Optional[str] = Field(
        default=None, max_length=3, description="Incoterms part 1 (e.g., FOB)"
    )
    incoterms2: Optional[str] = Field(
        default=None, max_length=28, description="Incoterms part 2 (location)"
    )
...
class InboundOrderMessage(BaseModel):
    ...
    message_id: str = Field(max_length=36, description="Unique message ID (replaces IDoc DOCNUM)")
    message_type: str = Field(
        default="ORDERS", max_length=30, description="Message type (replaces MESTYP)"
    )
    sender_system: Optional[str] = Field(
        default=None, max_length=30, description="Sending system identifier"
    )
...
class OrderSyncResult(BaseModel):
    ...
    order_number: Optional[str] = Field(
        default=None, max_length=10, description="Created sales order number (VBELN)"
    )
