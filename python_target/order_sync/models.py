...
class OrderHeader(BaseModel):
    ...
    customer_po_number: Optional[str] = Field(
        default=None, max_length=35, description="Customer PO reference (BSTKD)"
    )
...
class InboundOrderMessage(BaseModel):
    ...
    message_id: str = Field(max_length=70, description="Unique message ID (replaces IDoc DOCNUM)")
...
class OrderSyncResult(BaseModel):
    ...
    message_id: str = Field(max_length=70)
    status: OrderStatus
