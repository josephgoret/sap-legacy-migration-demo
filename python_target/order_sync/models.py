...
class OrderItem(BaseModel):
    ...
    unit_of_measure: str = Field(default="EA", max_length=5, description="Unit of measure (MENEE)")
...
class OrderHeader(BaseModel):
    ...
    incoterms1: Optional[str] = Field(
        default=None, max_length=5, description="Incoterms part 1 (e.g., FOB)"
    )
    incoterms2: Optional[str] = Field(
        default=None, max_length=40, description="Incoterms part 2 (location)"
    )
...
class InboundOrderMessage(BaseModel):
    ...
    sender_system: Optional[str] = Field(
        default=None, max_length=20, description="Sending system identifier"
    )
